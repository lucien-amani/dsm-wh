...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    <?php
    $stmt = $pdo->query("SELECT id, slug, updated_at FROM news WHERE published = 1 ORDER BY created_at DESC");
    while($row = $stmt->fetch()):
        $hash = $hashids->encode($row['id']);
        $url = SITE_URL . "/view/news/" . $hash . "/" . $row['slug'];
    ?>
    <url>
    ...
    <!-- Projets Dynamiques -->
    <?php
    $stmt = $pdo->query("SELECT id, slug, updated_at FROM projects WHERE published = 1 ORDER BY created_at DESC");
    while($row = $stmt->fetch()):
        $hash = $hashids->encode($row['id']);
        $url = SITE_URL . "/view/projects/" . $hash . "/" . $row['slug'];
    ?>
    <url>
        <loc><?php echo htmlspecialchars($url); ?></loc>
        <lastmod><?php echo date('Y-m-d', strtotime($row['updated_at'] ?? 'now')); ?></lastmod>
