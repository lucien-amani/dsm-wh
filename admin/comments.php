...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    try {
        ...
        error_log("Erreur modération commentaire (ID $id, Action $action) : " . $e->getMessage());
        $error_msg = urlencode("Une erreur est survenue lors de l'opération de modération.");
        header("Location: " . SITE_URL . "/admin/commentaires?error=$error_msg&status=$status");
        exit;
    }
}
...
try {
    ...
    $db_error = "Impossible de récupérer la liste des commentaires pour le moment. Veuillez réessayer plus tard.";
}
    ...
    <main class="flex-1 lg:ml-72 flex flex-col min-w-0">
        ...
        <div class="p-10 space-y-10 animate-fade-in max-w-6xl mx-auto w-full">
            ...
            <div class="grid grid-cols-1 gap-8">
                <?php if ($db_error): ?>
                    <div class="glass-panel p-24 rounded-[4rem] text-center border-2 border-dashed border-rose-200 dark:border-rose-900/40 flex flex-col items-center">
                        <div class="w-24 h-24 bg-rose-50 dark:bg-rose-950/40 rounded-[2rem] flex items-center justify-center mb-8 text-rose-500">
                        ...
                        <h3 class="text-2xl font-black text-rose-600 dark:text-rose-400 uppercase tracking-tighter mb-2">Erreur de chargement</h3>
                        <p class="text-slate-500 dark:text-slate-400 font-medium italic max-w-lg"><?php echo htmlspecialchars($db_error); ?></p>
                    </div>
