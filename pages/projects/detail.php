<?php
// pages/projects/detail.php
// Ce fichier remplace l'ancien project-detail.php à la racine et utilise l'ID décrypté

if (!isset($id)) {
    // Si l'ID n'est pas fourni par le routeur, c'est une erreur -> redir vers liste
    // ou 404
    header('Location: ' . $router->generate('projects_list'));
    exit;
}
...
$stmt = $pdo->prepare("SELECT * FROM projects WHERE id = :id AND published = 1");
$stmt->execute([':id' => $id]);
$project = $stmt->fetch();

if (!$project) {
    header($_SERVER["SERVER_PROTOCOL"] . " 404 Not Found");
    include 'pages/404.php'; // Ou affichage erreur simple
    exit;
}

// Vérifier et forcer l'URL canonique si le slug est incorrect
// $original_slug est fourni par le routeur
if (isset($original_slug) && $original_slug !== $project['slug']) {
    forceCanonicalUrl('projects', $id, $project['slug']);
}
...
$page_title = htmlspecialchars($project['title']) . ' - Dynamique Samy Magadju/Wema ni Hakiba';
$page_description = htmlspecialchars($project['description']);
$page_image = $project['image'] ? (strpos($project['image'], 'http') === 0 ? $project['image'] : SITE_URL . '/uploads/projects/' . $project['image']) : SITE_URL . '/assets/logo/logo-dsm.jpg';

// Variables SEO pour JSON-LD (Utilisons NewsArticle pour la visibilité Google même pour les projets)
$is_article = true;
$article_title = $project['title'];
$article_date = date('c', strtotime($project['created_at']));
$article_author = 'Dynamique Samy Magadju';
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="mb-8 text-sm">
            <ol class="flex items-center gap-2 text-white/70">
                <li><a href="<?php echo SITE_URL; ?>/" class="hover:text-white">Accueil</a></li>
                <li>›</li>
                <li><a href="<?php echo $router->generate('projects_list'); ?>" class="hover:text-white">Projets</a></li>
                <li>›</li>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2">
                ...
                <?php if (!empty($project['embed_code'])): ?>
                    <div class="mb-12 rounded-3xl overflow-hidden shadow-2xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-center w-full max-w-4xl mx-auto">
